...
static char *handle_detect_changes(cbm_mcp_server_t *srv, const char *args) {
    ...
    /* 'since' takes precedence over 'base_branch'. If neither is set,
     * default to 'HEAD~1' (most recent commit) so the tool has useful
     * default behavior — comparing against 'main' returns nothing when
     * already on the main branch. */
    if (!base_branch && !since) {
        base_branch = heap_strdup("HEAD~1");
    }
    if (since && since[0]) {
        free(base_branch);
        base_branch = since;
        since = NULL; /* ownership transferred */
    }
    free(since);
