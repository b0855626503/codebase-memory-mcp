...
static void cbm_install_session_reminder_script(const char *home) {
    ...
    (void)fprintf(
        f, "#!/bin/bash\n"
           ...
           "cat << 'REMINDER'\n"
           "CRITICAL - Code Discovery Protocol (3 tiers):\n"
           "\n"
           "TIER 1 — Codebase Memory MCP (structure + relations):\n"
           "   - search_graph(name_pattern/label/qn_pattern) to find functions/classes/routes\n"
           "   - trace_path(function_name, mode=calls|data_flow|cross_service) for call chains\n"
           "   - get_code_snippet(qualified_name) for exact symbol source\n"
           ...
           "   - If project is not indexed, run index_repository FIRST\n"
           "\n"
           "   SMART TOOLS (auto-analysis + incident tracking):\n"
           "   - smart_analyze: full auto-pipeline — architecture + rules + incidents + ADR\n"
           "   - analyze_architecture_reasoning: deep 7-dimension architecture analysis\n"
           "   - detect_schema_drift: detect structural changes vs baseline\n"
           "   - save_baseline: save a schema baseline snapshot\n"
           "   - detect_dead_code: find potentially dead code\n"
           "   - check_architecture_rules: verify architecture against configurable rules\n"
           "   - create_incident / list_incidents: incident memory for bugs/outages\n"
           "   - ingest_traces: runtime trace data → RUNTIME_CALLS graph edges\n"
           "\n"
           "TIER 2 — Serena MCP (LSP-fresh code reading + editing):\n"
           "   - get_symbols_overview → find_symbol → for reading code files\n"
           "   - find_referencing_symbols → who uses this?\n"
           "   - replace_content / replace_symbol_body → for editing\n"
           "   - Read/Edit on code files → FORBIDDEN; use Serena instead\n"
           "\n"
           "TIER 3 — Grep/Glob/Read (non-code only):\n"
           "   - Grep: text search in non-code (configs, .env, docs)\n"
           "   - Glob: find files by name\n"
           "   - Read: non-code files (config, docs, .json, .env)\n"
           "   - Read before Edit is still required for non-code files\n"
           "REMINDER\n");
#ifndef _WIN32
    fchmod(fileno(f), CLI_OCTAL_PERM);
#endif
